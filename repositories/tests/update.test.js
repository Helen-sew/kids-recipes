...
describe('recipeRepository.update', () => {
    ...
    it('should update an existing object and return an updated object', async() => {
        const updatedRecipe = await recipeRepository.update('Vanilla cupcakes', {
    ...
    "comments": ""
        ...
        expect(updatedRecipe.result.n).to.equal(1);
    });

    it('should return error when item to be updated does not exist', async() => {
        try{
        const updatedRecipe = await RecipeRepository.update('Monster', {
            "title": "Sticky pork lettuce wraps",
            "img": "https://www.bbcgoodfood.com/sites/default/files/styles/recipe/public/recipe/recipe-image/2018/04/sticky-pork-lettuce-wraps.jpg?itok=1amjObj6",
            "ingredients": [
